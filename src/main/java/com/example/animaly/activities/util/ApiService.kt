...
interface ApiService {
    ...
    @FormUrlEncoded
    @POST("carte/ajouter")
    fun ajouterCarte(@FieldMap params: HashMap<String?, String?>): Call<JsonObject>

    @GET("carte/{user}")
    fun getCarte(@Path("user") user_id: String): Call<JsonObject>

    @FormUrlEncoded
    @POST("carte/verifier_cvv")
    fun verifierCarte(@Field("user") user_id : String, @Field("cvv") cvv: String): Call<JsonObject>

    @FormUrlEncoded
    @POST("commande/ajouter")
    fun ajoutercommande(@Field("user") user_id : String, @Field("total") total: String): Call<JsonObject>

    @GET("commande/all/{user}")
    fun getUserCommandes(@Path("user") user_id: String): Call<JsonObject>
